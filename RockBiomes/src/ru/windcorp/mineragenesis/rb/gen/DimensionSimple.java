...
public class DimensionSimple extends Dimension {
	
	private final DiscreteField2D<RockBiomeType> regolith;

	public DimensionSimple(
			String name, int id,
			DiscreteField2D<RockBiomeType> regolith,
			long seed) {
		super(name, id, seed);
	...
	public static DimensionSimple build(Arguments args) throws ConfigurationException {
		return new DimensionSimple(
	...
	public void processChunk(GenerationRequest request, Workspace w) {
		...
		for (int xInChunk = 0; xInChunk < CHUNK_SIZE; ++xInChunk) {
			...
			for (int zInChunk = 0; zInChunk < CHUNK_SIZE; ++zInChunk) {
				...
				for (block.yInt = 1; block.yInt <= maxHeight; ++block.yInt) processBlock(cacheRegolith, w, request);
			}
		}
	}
